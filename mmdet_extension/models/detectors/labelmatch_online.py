...
from mmdet_extension.core.utils.dist_utils import concat_all_gather
from mmdet_extension.models.detectors import SemiTwoStageDetector


@DETECTORS.register_module()
class LabelMatchOnline(SemiTwoStageDetector):
    def __init__(self,
                 backbone,
                 rpn_head,
                 roi_head,
        ...
        self.queue_num = 8 if self.debug else cfg.get('queue_num', 128 * 100)
        # self.queue_num = 8 if self.debug else 128 * 100  # image num
        ...
        self.update_score_iter = 1
        self.start_update = False

        self.cls_ratio_gt = None
        self.boxes_per_image_gt = None
    ...
    def forward_train_semi(
        ...
        gt_bboxes_pred, gt_labels_pred, gt_bboxes_ig_pred, gt_labels_ig_pred, score = \
        ...
        self._dequeue_and_enqueue(score)  # update score queue
        if self.debug:
    ...
    def create_pseudo_results(self, img, bbox_results, box_transform, device,
        ...
        score_list = [[] for _ in self.CLASSES]  # for score queue
        _, _, h, w = img.shape
        ...
        for b, result in enumerate(bbox_results):
            ...
            for cls, r in enumerate(result):
                ...
                if len(r[:, -1]) >= self.max_cls_per_img:
                    score = np.sort(r[:, -1])[::-1]  # this line may not necessary
                    score = score[:self.max_cls_per_img]
                else:
                    score = np.r_[r[:, -1], np.zeros(self.max_cls_per_img - len(r))]
        ...
        score = np.array([np.concatenate(s) for s in score_list]).astype(np.float32)
        score = torch.from_numpy(score).transpose(0, 1).contiguous().to(device)
        return gt_bboxes_pred, gt_labels_pred, gt_bboxes_ig_pred, gt_labels_ig_pred, score
    ...
    def _update_cls_thr(self):
        if self.cur_iter % self.update_score_iter == 0:
            boxes_per_img = self.boxes_per_image_gt * self.percent
            pos_location = self.queue_num * boxes_per_img * self.cls_ratio_gt
            ig_location = self.queue_num * self.boxes_per_image_gt * self.cls_ratio_gt
            score, _ = self.score_queue.sort(dim=0, descending=True)
            for cls in range(len(self.CLASSES)):
                self.cls_thr[cls] = max(0.05, score[int(pos_location[cls]), cls].item())
                self.cls_thr_ig[cls] = max(0.05, score[int(ig_location[cls]), cls].item())
            if self.cur_iter % 1000 == 0:
                logger = get_root_logger()
                info = ' '.join([f'({v:.2f}-{self.CLASSES[i]})' for i, v in enumerate(self.cls_thr)])
                logger.info(f'update score thr (positive): {info}')
                info = ' '.join([f'({v:.2f}-{self.CLASSES[i]})' for i, v in enumerate(self.cls_thr_ig)])
